...
import { Download, Share2, Mail, MessageCircle, Link, Scroll, Crown, Star, Shield, Award } from 'lucide-react';
...
const LetterGenerator = () => {
  ...
  const generateInvitationContent = () => {
    ...
    return {
      ...
      invitation: `ROYAL DECREE OF INVITATION`,
      subtitle: `By the Authority of UNCIF Council of Excellence`,
      message: `We, the Royal Council of UNCIF (Uniford National Council of Institutes & Frontliners), hereby extend this Sacred Invitation to join our Distinguished Order of Digital Artisans and Innovation Leaders.`,
      purpose: `Your Excellency has been selected to receive our Royal Academy's comprehensive support services:`,
      benefits: [
        ...
        'Royal Documentation & Legal Services for Your Organization',
      ...
      mission: `Our Sacred Mission addresses the critical need whereby 68% of noble initiators and founders lack proper digital presence due to financial constraints or insufficient technical awareness. We stand committed to serving distinguished leaders like yourself with unwavering dedication and excellence.`,
      ...
      callToAction: `To accept this Royal Invitation and begin your journey with UNCIF, please contact our Royal Council through the channels provided below. We eagerly await the honor of serving your distinguished organization.`,
      date: currentDate,
      signature: 'By Royal Decree and Sacred Oath',
      council: 'The UNCIF Royal Council of Digital Excellence'
    };
  };
  ...
  const handleDownload = async (format: 'pdf' | 'png' | 'jpg') => {
    ...
    try {
      // Configure for A4 size (210mm x 297mm = 794px x 1123px at 96 DPI)
      ...
      if (format === 'pdf') {
        ...
        pdf.save(`UNCIF_Royal_Invitation_${formData.name || 'Document'}.pdf`);
      } else {
        const link = document.createElement('a');
        link.download = `UNCIF_Royal_Invitation_${formData.name || 'Document'}.${format}`;
        link.href = canvas.toDataURL(`image/${format}`, 1.0);
        link.click();
      }
      
      console.log(`Royal invitation downloaded successfully as ${format.toUpperCase()}`);
    } catch (error) {
  ...
  const handleShare = (method: 'email' | 'whatsapp' | 'link') => {
    ...
    switch (method) {
      ...
      case 'link':
        ...
        alert('Royal invitation has been copied to your clipboard!');
        break;
    }
  };
  ...
  return (
    <>
      <SEO 
        ...
        description="Create professional royal-style invitation letters for UNCIF's comprehensive tech support program. Join our distinguished order of digital artisans."
        ...
        <div className="bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600 text-white py-8 sm:py-16 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            <div className="flex flex-col sm:flex-row items-center justify-center mb-4 sm:mb-8">
              <Crown className="w-12 h-12 sm:w-16 sm:h-16 text-yellow-200 mb-4 sm:mb-0 sm:mr-4" />
              <div>
                ...
                <p className="text-lg sm:text-xl md:text-2xl text-amber-100 font-medium">
                  Sacred Decree for Tech Support Excellence
                </p>
              </div>
              <Crown className="w-12 h-12 sm:w-16 sm:h-16 text-yellow-200 mt-4 sm:mt-0 sm:ml-4 hidden sm:block" />
            </div>
            <p className="text-sm sm:text-base md:text-lg text-amber-50 max-w-4xl mx-auto leading-relaxed px-4">
              ...
              Let UNCIF bestow upon you the gift of technological mastery and professional excellence.
            </p>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6 lg:gap-12">
            ...
            <Card className="border-2 sm:border-4 border-amber-400 shadow-2xl bg-gradient-to-br from-amber-50 to-yellow-50 order-2 lg:order-1">
              ...
              <CardContent className="p-4 sm:p-8 space-y-4 sm:space-y-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="techChallenges" className="text-base sm:text-lg font-semibold text-purple-900">Technology Requirements</Label>
                  <Textarea
                    id="techChallenges"
                    value={formData.techChallenges}
                    onChange={(e) => handleInputChange('techChallenges', e.target.value)}
                    placeholder="Describe your digital needs (website development, portal creation, technical support, system integration...)"
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Any special message or requirements for your royal invitation..."
                    className="border-2 border-amber-300 focus:border-purple-500 min-h-[80px] sm:min-h-[100px] text-base sm:text-lg p-3 sm:p-4"
                  />
                </div>
              </CardContent>
            ...
            {/* A4 Formatted Invitation Preview */}
                ...
                <div className="overflow-auto max-h-[600px] sm:max-h-[800px]">
                  <div 
                    ...
                    style={{
                      width: '794px',
                      height: '1123px',
                      background: 'linear-gradient(45deg, #f8f4e6 0%, #f5f5f0 50%, #f8f4e6 100%)',
                  ...
                  >
                    {/* A4 Document Content */}
                    <div className="absolute inset-8 border-8 border-double border-purple-700 rounded-lg bg-gradient-to-br from-purple-50 to-indigo-50">
                      {/* Decorative Elements */}
                      <div className="absolute top-4 left-4 w-12 h-12 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-full flex items-center justify-center shadow-lg">
                        <Star className="w-6 h-6 text-purple-800" />
                      </div>
                      <div className="absolute top-4 right-4 w-12 h-12 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-full flex items-center justify-center shadow-lg">
                        <Award className="w-6 h-6 text-purple-800" />
                      </div>
                      <div className="absolute bottom-4 left-4 w-12 h-12 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-full flex items-center justify-center shadow-lg">
                        <Shield className="w-6 h-6 text-purple-800" />
                      </div>
                      <div className="absolute bottom-4 right-4 w-12 h-12 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-full flex items-center justify-center shadow-lg">
                      ...
                      <div className="p-12 space-y-8">
                        ...
                        <div className="text-center">
                          <Crown className="w-20 h-20 text-amber-600 mx-auto mb-6" />
                          ...
                          <div className="text-lg text-purple-600 italic">Royal Academy of Digital Excellence</div>
                          <div className="w-40 h-1 bg-gradient-to-r from-purple-600 to-amber-500 mx-auto mt-6"></div>
                        </div>

                        {/* Royal Decree Title */}
                        <div className="text-center bg-gradient-to-r from-amber-100 to-yellow-100 p-6 rounded-lg border-4 border-amber-400 shadow-lg">
                          <h1 className="text-3xl font-bold text-purple-900 mb-2">{content.invitation}</h1>
                        ...
                        {/* Signature Section */}
                        <div className="text-center pt-8 border-t-2 border-purple-300">
                          ...
                          <Crown className="w-12 h-12 text-amber-600 mx-auto mt-4" />
                        </div>
                      </div>
                    </div>
                  </div>
